...
import (
	...
	"github.com/MohitPanchariya/Snippet-Box/internal/validator"
...
func (app *application) userSignupPost(w http.ResponseWriter, r *http.Request) {
	...
	form.CheckField(validator.Matches(form.Email, validator.EmailRX), "email", "This field must contain a valid email")
	...
	form.CheckField(validator.MinChars(form.Password, 8), "password", "This field must be 8 characters long at least")
...
func (app *application) userLoginPost(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		if errors.Is(err, models.ErrInvalidCredentials) {
			...
			// Since either the email or the password could be wrong,
			// the email doesn't need to be retained
			form.Email = ""
			data := app.newTemplateData(r)
			data.Form = form
			app.render(w, http.StatusUnprocessableEntity, "login.tmpl.html", data)
	...
	// Generate a new session id to prevent session fixing attacks
